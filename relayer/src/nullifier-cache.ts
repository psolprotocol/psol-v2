...
export class NullifierCache {
  ...
  private config: Required<NullifierCacheConfig>;
   ...
   */
  private buildKey(nullifierHash: Uint8Array | string): string {
    const hashHex =
   ...
   */
  async get(nullifierHash: Uint8Array | string): Promise<boolean | null> {
    if (!this.isAvailable()) {
   ...
   * @param nullifierHash - The nullifier hash (32 bytes or hex string)
   */
  async markSpent(nullifierHash: Uint8Array | string): Promise<void> {
    if (!this.isAvailable()) {
   ...
   */
  async getStats(): Promise<{ enabled: boolean; connected: boolean; keyCount?: number }> {
    const stats = {
 ...
 */
export function createDisabledCache(): NullifierCache {
