...
describe('classifyError', () => {
  describe('TRANSIENT_RPC errors', () => {
    ...
    test.each(transientErrors)('classifies "%s" as TRANSIENT_RPC', (message) => {
      ...
      expect(classifyError(error)).toBe(ErrorCategory.TRANSIENT_RPC);
    });
  });
  ...
  describe('STATE_CONFLICT errors', () => {
    ...
    test.each(stateConflictErrors)('classifies "%s" as STATE_CONFLICT', (message) => {
      const error = new Error(message);
      expect(classifyError(error)).toBe(ErrorCategory.STATE_CONFLICT);
    });
  });
...
describe('isRetryableCategory', () => {
  it('returns true for TRANSIENT_RPC', () => {
    expect(isRetryableCategory(ErrorCategory.TRANSIENT_RPC)).toBe(true);
  });

  it('returns true for UNKNOWN (might be transient)', () => {
    expect(isRetryableCategory(ErrorCategory.UNKNOWN)).toBe(true);
  });
  ...
  it('returns false for STATE_CONFLICT', () => {
    expect(isRetryableCategory(ErrorCategory.STATE_CONFLICT)).toBe(false);
  });

  it('returns false for RESOURCE', () => {
    expect(isRetryableCategory(ErrorCategory.RESOURCE)).toBe(false);
  });
});
...
describe('Simulated Retry Behavior', () => {
  ...
  async function simulateRetryLoop(
    attemptFn: (attempt: number) => Promise<string>,
    options: {
      maxAttempts?: number;
      overallTimeoutMs?: number;
    } = {}
  ): Promise<{ result?: string; error?: Error; attempts: number }> {
  ...
  it('respects overall timeout', async () => {
    const { error, attempts } = await simulateRetryLoop(
      async () => {
        throw new Error('Connection timeout');
      },
...
describe('Edge Cases', () => {
  ...
  it('is case-insensitive for error matching', () => {
    ...
    expect(classifyError(upperError)).toBe(ErrorCategory.VALIDATION);
    expect(classifyError(lowerError)).toBe(ErrorCategory.VALIDATION);
    expect(classifyError(mixedError)).toBe(ErrorCategory.VALIDATION);
  });
