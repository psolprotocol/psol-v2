...
fn load_vectors() -> Vectors {
    let json = include_str!(concat!(env!("CARGO_MANIFEST_DIR"), "/../../tools/poseidon-vectors/vectors.json"));
...
fn poseidon2_vectors() {
    ...
    for (i, c) in v.poseidon2.iter().enumerate() {
        ...
        assert_eq!(got, expected, 
            ...
            i, c.input[0], c.input[1], c.out, scalar_to_hex(&got));
...
fn sensitivity_test_detects_changes() {
    ...
    assert_eq!(actual, expected, 
        ...
        scalar_to_hex(&expected), scalar_to_hex(&actual));
...
fn hardcoded_vectors() {
    ...
    let mut three = [0u8; 32]; three[31] = 3;
    let mut four = [0u8; 32]; four[31] = 4;
    
    // poseidon([0,0]) from circomlibjs
    let h = poseidon2(&zero, &zero).unwrap();
    assert_eq!(scalar_to_hex(&h), "0x2098f5fb9e239eab3ceac3f27b81e481dc3124d55ffed523a839ee8446b64864");
    println!("  poseidon([0,0]) ✓");
    
    // poseidon([1,2]) from circomlibjs
    let h = poseidon2(&one, &two).unwrap();
    assert_eq!(scalar_to_hex(&h), "0x115cc0f5e7d690413df64c6b9662e9cf2a3617f2743245519e19607a4417189a");
    println!("  poseidon([1,2]) ✓");
    
    // poseidon([1,2,3]) from circomlibjs
    let h = poseidon3(&one, &two, &three).unwrap();
    assert_eq!(scalar_to_hex(&h), "0x0e7732d89e6939c0ff03d5e58dab6302f3230e269dc5b968f725df34ab36d732");
    println!("  poseidon([1,2,3]) ✓");
    
    // poseidon([1,2,3,4]) from circomlibjs
    let h = poseidon4(&one, &two, &three, &four).unwrap();
    assert_eq!(scalar_to_hex(&h), "0x299c867db6c1fdd79dcefa40e4510b9837e60ebb1ce0663dbaa525df65250465");
    println!("  poseidon([1,2,3,4]) ✓");
    
    // poseidon([0,0,0,0]) from circomlibjs
    let h = poseidon4(&zero, &zero, &zero, &zero).unwrap();
    assert_eq!(scalar_to_hex(&h), "0x0532fd436e19c70e51209694d9c215250937921b8b79060488c1206db73e9946");
