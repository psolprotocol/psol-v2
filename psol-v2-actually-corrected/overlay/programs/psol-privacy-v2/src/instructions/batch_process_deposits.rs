//! Batch Process Deposits Instruction - ROBUST PDA ENFORCEMENT
//!
//! # Security Fix: Manual PDA Verification
//!
//! Previous version used Anchor's Option<Account> with bump constraint.
//! This is fragile and can fail across Anchor versions.
//!
//! This version does MANUAL PDA derivation and comparison for robustness.
//!
//! # Authorization Flow
//!
//! 1. Is batcher == pool authority? → Authorized
//! 2. Else:
//!    a. Require batcher_role account provided
//!    b. Derive expected PDA from seeds
//!    c. Compare expected PDA to provided account address
//!    d. Reject if mismatch
//!    e. Check is_enabled flag
//!
//! This is Anchor-version-proof and unambiguous.
...
pub fn handler(
    ...
    require!(
        tree_used + to_process <= tree_capacity,
    ...
    emit!(BatchProcessedEvent {
        ...
        batcher,
        batch_size: actual_count as u32,
        start_leaf_index,
        end_leaf_index,
